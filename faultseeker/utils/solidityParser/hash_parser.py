from solidityParser.antlrGenerated.SolidityLexer import SolidityLexer
from antlr4 import *
from solidityParser.constantTokenId import *
import traceback
import tlsh
...
def hashString(content):
    # TLSH requires a minimum length of content to generate a hash
    min_length = 50
    if len(content) < min_length:
        # Pad the content if it's too short
        content += "0" * (min_length - len(content))
    # Compute the TLSH hash
    tlsh_hash = tlsh.hash(content.encode('utf-8'))
    return tlsh_hash
    ...
    try:
        ...
        while i < length:
            ...
            if id is not None:
                ...
                normalized_output += content
            i += 1
        return {'hash':hashString(normalized_output), 'contracts': subcontracts, 'functions': functions, 'output':original_output}
    except Exception:
        traceback.print_exc()
        return {'hash':hashString(normalized_output), 'contracts': subcontracts, 'functions': functions, 'output':original_output}
...
if __name__ == '__main__':
    print(parseFile('./FlashLoanExample.sol'))